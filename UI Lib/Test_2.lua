--[[
     HuntaDev 18/01/2021
	 Credits to Bytes#0001 for the UI library https://www.roblox.com/users/87601985/profile/
	 Only the CoreGUI name was changed so don't cry about that I customized the shit and give my self credits
	 also what are you doing here my fellow friend?
--]]
local function DestroyYep()
    for x = 1,69 do 
        if game.CoreGui:FindFirstChild("Q8#C+=-Cj5S+*5-*7^h*K-KS+tQ6B") then game.CoreGui:FindFirstChild("Q8#C+=-Cj5S+*5-*7^h*K-KS+tQ6B"):Destroy() end
    end
end
...
function Library:CreateWindow(windowname,windowinfo)
    local DeltaVape = Instance.new("ScreenGui")
    ...
    DeltaVape.Name = "Q8#C+=-Cj5S+*5-*7^h*K-KS+tQ6B"
    DeltaVape.Parent = game.CoreGui
    DeltaVape.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    DeltaVape.ResetOnSpawn = false
    
    Frame.Parent = DeltaVape
...
-----------------------------------------------------
|                    Delta Hub                      |
|  @HuntaDev - Creator of Delta and Main Dev        |
|  @Derek    - Creator of the UI LIB that Delta use | 
|  @You      - Using Delta!                         |
|                                                   |
|                                                   |
|              Thank you for using Delta            |
|        My first UI that I have made of in         |
|                                                   |
|                                                   |
|                                                   |
-----------------------------------------------------]])
